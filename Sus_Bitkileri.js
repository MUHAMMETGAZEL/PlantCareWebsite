/*document.addEventListener('DOMContentLoaded', function() {
    const bitkiler = [
        { adi: 'Gül', bilimselAdi: 'Rosa', toprak: 'İyi drene toprak', sulama: 'Haftada 2 kez', gunes: 'Tam güneş', foto: 'https://example.com/elma.jpg' },
        { adi: 'Orkide', bilimselAdi: 'Orchidaceae', toprak: 'Orkide toprak', sulama: 'Haftada 1 kez', gunes: 'Dolaylı ışık', foto: 'https://example.com/domates.jpg' },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadTableData() {
        fetch('http://localhost:5000/bitkiler')
...
window.loadTableData = loadTableData;
    ...
    window.saveRow = function(index) {
        ...
        fetch(`http://localhost:5000/bitkiler${bitkiler[index].id ? '/' + bitkiler[index].id : ''}`, {
    ...
    window.deleteRow = function(index) {
        const bitki = bitkiler[index];
        if (bitki.id) {
            fetch(`http://localhost:5000/bitkiler/${bitki.id}`, {
                method: 'DELETE'
            }).then(response => {
                if (response.ok) {
                    loadTableData();
