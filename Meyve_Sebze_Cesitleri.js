/*document.addEventListener('DOMContentLoaded', function() {
    const bitkiler = [
        { adi: 'Elma', bilimselAdi: 'Malus domestica', toprak: 'Tınlı toprak', sulama: 'Haftada 1-2 kez', gunes: 'Tam güneş', foto: 'https://example.com/elma.jpg' },
        { adi: 'Domates', bilimselAdi: 'Solanum lycopersicum', toprak: 'Hafif toprak', sulama: 'Haftada 2-3 kez', gunes: 'Tam güneş', foto: 'https://example.com/domates.jpg' },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadTableData() {
        fetch('http://localhost:5001/bitkiler')
...
window.loadTableData = loadTableData;
    ...
    window.saveRow = function(index) {
        ...
        fetch(`http://localhost:5001/bitkiler${bitkiler[index].id ? '/' + bitkiler[index].id : ''}`, {
    ...
    window.deleteRow = function(index) {
        const bitki = bitkiler[index];
        if (bitki.id) {
            fetch(`http://localhost:5001/bitkiler/${bitki.id}`, {
                method: 'DELETE'
            }).then(response => {
                if (response.ok) {
                    loadTableData();
